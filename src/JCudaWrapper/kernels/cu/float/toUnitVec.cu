...
extern "C" __global__ void toUnitVecKernel(
    const int n, 
    float* srcVecs, const int heightSrc, const int ldSrc,
    float* dstVecs, const int heightDst, const int ldDst
) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx >= n) return;

    const float* src = srcVecs + ind(idx, ldSrc, heightSrc);
    float* dst = dstVecs + ind(idx, ldDst, heightDst);

    float mag = sqrt(src[0] * src[0] + src[1] * src[1] + src[2] * src[2]);
