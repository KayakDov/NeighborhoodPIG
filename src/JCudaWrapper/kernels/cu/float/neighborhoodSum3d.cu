...
class Pixel {
private:
    float* srcMat;   /**< Pointer to the source matrix. */
    const int srcStride;        /**< Step size for moving along the specified direction. */
    float* dstMat;   /**< Pointer to the target matrix. */    
...
public:
     ...
     */
    __device__ Pixel(float* srcMat, float* dstMat, const int srcStride, const int dstStride)
     ...
     */
    __device__ float sourceValue(int offset = 0) const {
     ...
     */
    __device__ float& targetValue(int offset = 0) {
