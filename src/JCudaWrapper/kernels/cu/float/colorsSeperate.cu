 ...
 */
__device__ int down(int a, float theta) {
 ...
 */
__device__ int up(int a, float theta) {
...
public:
     ...
     */
    __device__ Writer(unsigned char* red, unsigned char* green, unsigned char* blue, float intensity)
...
extern "C" __global__ void colorsSeperateKernel(
    int n, 
    const float* angles, const int anglesInc, 
    unsigned char* reds, unsigned char* greens, unsigned char* blues,
    const float* intensities, int intensitiesInc
) {
    ...
    float angle = angles[idx * anglesInc];
    float intensity = (intensitiesInc != -1) ? intensities[idx * intensitiesInc] : 1.0;
