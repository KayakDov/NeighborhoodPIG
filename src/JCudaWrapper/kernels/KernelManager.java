...
public class KernelManager implements AutoCloseable {
     ...
     * arguments.
     */
    public void run(String name, Handle handle, ThreadCount numThreads, Pointer... additionalArguments) {
     ...
     */
    public void run(String name, Handle handle, ThreadCount numThreads, PArray2dTo2d[] arrays, Dimensions dim, Pointer... additionalParmaters) {
     ...
     * @param additionalParmaters Any additional parameters.
     */
    public void run(String name, Handle handle, PArray2dTo2d[] arrays, Dimensions dim, Pointer... additionalParmaters) {

        new Kernel(name).run(handle, dim.threadCount(), arrays, dim.getGpuDim(), additionalParmaters);
    }
    ...
    public class Kernel {
         ...
         * @param handle
         * @param threadCount The number of threads in each dimension.
         * @param additionalParmaters These should all be pointers to cpu arrays
         * or pointers to device pointers.
         */
        public <T extends Array> void run(Handle handle, ThreadCount threadCount, Pointer... additionalParmaters) {

            Pointer kernelParameters = Pointer.to(additionalParmaters);
            
            checkResult(JCudaDriver.cuLaunchKernel(
                    function,
                    threadCount.xBlocksPerGrid(), threadCount.yBlocksPerGrid(), threadCount.zBlocksPerGrid(), // Grid size (number of blocks)
                    threadCount.xThreadsPerBlock(), threadCount.yThreadsPerBlock(), threadCount.zThreadsPerBlock(), // Block size (number of threads per block)
         ...
         */
        public void run(Handle handle, ThreadCount numThreads, PArray2dTo2d[] arrays, IArray dim, Pointer... additionalParmaters) {
